...
namespace Sample
{
    ...
    {
        // Prepare output file
        static System.IO.StreamWriter file = new System.IO.StreamWriter("fingerprintResults.txt");
        ...
        {
            file.WriteLine("Enrolling {0}...", name);

            // Initialize empty fingerprint object and set properties
            MyFingerprint fp = new MyFingerprint();
            fp.Filename = filename;
            // Load image from the file
            file.WriteLine(" Loading image from {0}...", filename);
            BitmapImage image = new BitmapImage(new Uri(filename, UriKind.RelativeOrAbsolute));
            fp.AsBitmapSource = image;
            // Above update of fp.AsBitmapSource initialized also raw image in fp.Image
            // Check raw image dimensions, Y axis is first, X axis is second
            file.WriteLine(" Image size = {0} x {1} (width x height)", fp.Image.GetLength(1), fp.Image.GetLength(0));
            ...
            file.WriteLine(" Extracting template...");
            Afis.Extract(person);
            // Check template size
            file.WriteLine(" Template size = {0} bytes", fp.Template.Length);
        ...
        static void Main(string[] args)
        {
            ...
            Afis.Threshold = 10;
            file.WriteLine("Identifying {0} in database of {1} persons...", probe.Name, database.Count);
            ...
            if (match == null)
            {
                file.WriteLine("No matching person found.");
                return;
            }
            // Print out any non-null result
            file.WriteLine("Probe {0} matches registered person {1}", probe.Name, match.Name);

            // Compute similarity score
            float score = Afis.Verify(probe, match);
            file.WriteLine("Similarity score between {0} and {1} = {2:F3}", probe.Name, match.Name, score);
